...
class MovingBatchNormNd(nn.Module):
    ...
    def forward(self, x, c=None, logpx=None, reg_states=tuple(), reverse=False):
        if reverse:
            return self._reverse(x, logpx, reg_states)
        else:
            return self._forward(x, logpx, reg_states)

    def _forward(self, x, logpx=None, reg_states=tuple()):
        ...
        if logpx is None:
            ...
            return y, logpx - self._logdetgrad(x, used_var).sum(-1, keepdim=True), reg_states

    def _reverse(self, y, logpy=None, reg_states=tuple()):
        ...
        if logpy is None:
            ...
            return x, logpy + self._logdetgrad(x, used_var).sum(-1, keepdim=True), reg_states
...
class MovingBatchNorm1d(MovingBatchNormNd):
    ...
    def forward(self, x, context=None, logpx=None, reg_states=tuple(), integration_times=None, reverse=False):
        ret = super(MovingBatchNorm1d, self).forward(
                x, context, logpx=logpx, reg_states=reg_states, reverse=reverse)
        return ret
